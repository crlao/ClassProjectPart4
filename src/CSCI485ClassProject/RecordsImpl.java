...
public class RecordsImpl implements Records{
  ...
  @Override
  public Cursor openCursor(String tableName, Mode mode) {
    Transaction tx = FDBHelper.openTransaction(db);
  ...
  @Override
  public Cursor openCursor(String tableName, String attrName, Object attrValue, ComparisonOperator operator, Mode mode, boolean isUsingIndex) {
    Transaction tx = FDBHelper.openTransaction(db);
  ...
  public StatusCode deleteRecord(Cursor cursor) {
    ...
    Cursor scanCursor =
        new Cursor(Mode.READ, cursor.getTableName(), cursor.getTableMetadata(), tx);
